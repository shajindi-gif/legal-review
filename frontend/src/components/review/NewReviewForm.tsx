...
const ACCEPTED_HUMAN = "PDF / Word / TXT / Markdown";

export type NewReviewFormProps = {
  /** 初始标题（可由 URL ?title= 预填） */
  presetTitle?: string;
  /** 提交后是否自动跳转（默认 true） */
  autoNavigate?: boolean;
  /** 自定义成功后的回调（如未跳转，可自取 taskId） */
  onSuccess?: (taskId: string) => void;
  /** 自定义"取消"按钮文本与动作（默认跳回上一页） */
  cancelLabel?: string;
  onCancel?: () => void;
  className?: string;
};
...
/**
 * 新建审查表单 ——  UI-M4 从 /upload 抽出，可被 /upload、/documents、⌘N 菜单复用。
 * 该组件是 LegalAI 当前唯一"开始任务"入口；它的复用点越多，upload→review 这条
 * 主路径就越一致。
 */
export function NewReviewForm({
  ...
  const [title, setTitle] = React.useState(presetTitle);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // 跨页联动 (UI-M5.5)：让 Home / Tasks / Reports / Documents 全部立即刷新
      qc.invalidateQueries({ queryKey: ["reviews"] });
      qc.invalidateQueries({ queryKey: ["all-task-documents"] });
      qc.invalidateQueries({ queryKey: ["quota"] });
      if (onSuccess) onSuccess(res.task_id);
      if (autoNavigate) router.push(`/review/${res.task_id}`);
    } catch (err) {
      ...
      <div className="flex flex-col gap-2">
        ...
        {!file ? (
          ...
          >
            ...
            <span className="text-xs text-gray-400">支持 {ACCEPTED_HUMAN}</span>
          </button>
        ) : (
